...
public class Constants {
    ...
    public static final String FIND_COLONIES_BY_STATE_ID_CITY_ID_CODE_POSTAL = "SELECT * FROM colonies WHERE colonies.city_id IN (SELECT DISTINCT ci.city_id FROM cities ci WHERE ci.state_id = :stateId AND ci.city_id = :cityId) AND colonies.postal_code = :codePostal";
    ...
    public static final String FIND_PRODUCTS_BY_NEW_PRODUCT_BRAND_SUB_BRAND = "SELECT * FROM products WHERE sub_brand_id IN (SELECT sub_brand_id FROM sub_brands WHERE sub_brand_name LIKE %:subBrandName% AND brand_id IN (SELECT brand_id FROM brands WHERE brand_name LIKE %:brandName%)) AND product_type_id IN (SELECT product_type_id FROM products_type WHERE type LIKE %:productType%) AND new_product = 1";
    public static final String FIND_PRODUCTS_BY_BRAND_SUB_BRAND = "SELECT * FROM products WHERE sub_brand_id IN (SELECT sub_brand_id FROM sub_brands WHERE sub_brand_name LIKE %:subBrandName% AND brand_id IN (SELECT brand_id FROM brands WHERE brand_name LIKE %:brandName%)) AND product_type_id IN (SELECT product_type_id FROM products_type WHERE type LIKE %:productType%)";
